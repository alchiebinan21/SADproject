		...
		if ($_REQUEST['payment']=='Submit Payment') 
		{
			...
			for($y=0;$y<$countLoanP;$y++)
			{
				...
				if($paymentNo == $payNo)
				{
					...
					if(floor($TotalToBePaid) == $payAmount)
					{
				...
				<form class="form-horizontal" role="form" method="get" name="payment"  action="paymentForm.php">
							<?php if($x == 0){?>
								<div class="form-group">
									...
									<div class="col-sm-3">
										<select name = "memberID" class="col-sm-0 form-control" id="exampleList">
...
<!-- Submit -->					
								...
								if (isset($_REQUEST['payment'])) 
									...
									<div class="form-group">
										...
										<div class="col-sm-3">
											<select name = "applicationNo" class="form-control col-sm-5" id="exampleList">
...
<!-- View Payments -->	
								...
								<?php }?>
										...
										<tr>
											...
											<td><p><?php echo number_format(floor($AmountArray[$y]/$TermArray[$y]),2);?></p></td> 
											<td><p><?php echo number_format($InterestArray[$y],2);?></p></td>
											<td><p id="loanp-<?php echo $dataLoanP[$y]->getpaymentNo();?>"><?php if($dataLoanP[$y]->getStatus() == "Paid Advance" || $dataLoanP[$y]->getStatus() == "Can be Paid Advance")
													{
														echo floor($AmountArray[$y]/$TermArray[$y]);
													}
													else if($dataLoanP[$y]->getStatus() == "Overdue")
													{
													  echo floor($AmountArray[$y]/$TermArray[$y] + $dataLoanPInterest[$y]);
													}
													
													else
													{
													  echo floor($AmountArray[$y]/$TermArray[$y] + $AmountArray[$y] * 0.02 * $TermArray[$y]/$TermArray[$y]);
													}
													?></p></td>
										...
										<div class="form-group">
											...
											<div class="col-sm-3">
												<select name = "paymentNo" class="form-control col-sm-2" id="paymentNoList">
							...
							<?php if (isset($_REQUEST['payment'])) 
									{
										...
										the sum of <b><?php echo number_format($ReceiptAmount,2);?></b> In Partial Payment for Loan
